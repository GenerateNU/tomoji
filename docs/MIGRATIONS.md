...
- `backfillCreatorUsernames` fills missing usernames and replaces exact
...
added after an earlier completed run. If the username backfill completed before
the random suffix change, reset it to revisit existing `creator_<userId>` values:

```bash
bunx convex run --deployment dev migrations:run \
  '{"fn":"migrations:backfillCreatorUsernames","reset":true}'
```

Add `"dryRun": true` to preview one batch without saving changes; preview each
migration separately, since a dry run does not execute the full sequence.
